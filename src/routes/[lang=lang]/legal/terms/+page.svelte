    ...
    jsonLdScript({
      ...
      '@graph': [
        webPage({
          ...
          name: `${data.t.legal.termsTitle} | ${data.t.site.name}`,
          description: `${data.t.legal.termsTitle} | ${data.t.site.name}`,
          dateModified: business.legalUpdated,
        }),
        breadcrumb([
          { name: data.t.nav.overview, url: absoluteUrl(`/${data.locale}/`) },
          { name: data.t.legal.termsTitle, url: absoluteUrl($page.url.pathname) },
        ]),
      ],
    })
  );
...
<svelte:head>
  <title>{data.t.legal.termsTitle} | {data.t.site.name}</title>
  <meta name="description" content={data.t.legal.termsLastUpdated} />
  ...
  <div class="max-w-3xl mx-auto px-8 relative z-10">
    <div class="text-center mb-2xl space-y-md">
      <h1 class="text-h1 font-display font-black tracking-tight text-on-surface">
        {data.t.legal.termsTitle}
      </h1>
      <p class="text-body-lg text-on-surface-variant leading-relaxed">
        {data.t.legal.termsLastUpdated}
      </p>
    </div>

    {#each data.t.legal.terms.sections as section}
